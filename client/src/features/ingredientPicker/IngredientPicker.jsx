...
import SearchIngredientsList from "./components/SearchIngredientList";
import FavoriteIngredientsList from "./components/FavoriteIngredientsList";
import FavoriteMealsPanel from "./components/FavoriteMealsPanel";
import MealBlock from "../mealPlanner/components/MealBlock";
import AddButton from "../../common/AddButton";

import { getFullUnitName } from "../../utilities/fullUnitNames";

import { addIngredientToMeal } from "../mealPlanner/mealsSlice";
import {
  addFavoriteMeal,
...
export default function IngredientPicker() {
  ...
  const handleAdd = () => {
    ...
    const ingredientName =
      selectedFood.food_name || selectedFood.name || "Ingredient";
          ...
          {selectedFood ? (
            <div className="flex-1 flex flex-col">
              ...
              <div>
                <p className="text-sm text-gray-500 text-center mb-6">
                  ...
                  (selectedFood.serving_unit === "g" ||
                    ...
                    ? `You're entering ${Math.round(
                        parseFloat(servingQty) || 0
                      )} grams`
                    : `1 ${getFullUnitName(
                        selectedFood.serving_unit
                      )} = ${Math.round(
              ...
              <div>
                <button
                  onClick={handleAdd}
                  disabled={!selectedFood || Number(servingQty) <= 0}
                  className={`w-full py-2 rounded text-white ${
                    !selectedFood || Number(servingQty) <= 0
            ...
            <div className="flex-1 flex items-center justify-center">
              <p className="text-center text-gray-500 px-4">
                Select an ingredient from the search or favorites to view
                details.
              </p>
            </div>
          )}
